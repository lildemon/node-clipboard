...
var kernel32 = ffi.Library('kernel32', {
  ...
  GlobalLock:   [ffi.ref.refType(ffi.ref.types.char), ['ulong']],
...
var user32 = ffi.Library('user32', {
  ...
  GetClipboardFormatNameA:       ['int', ['uint', 'pointer', 'int']],
...
GlobalHandle.prototype = {
  ...
  write: function write(input){
    ...
    if (typeof input === 'string') {
      ...
      var buf = pointer.reinterpret(size + 1);
      

      buf.writeCString( input );


      //console.log(size);
      //var bufstr = ffi.ref.allocCString(input);
      //console.log(bufstr);
      //pointer.writePointer(bufstr);
      //pointer.writePointer(bufstr);
    } else if (Buffer.isBuffer(input)) {
  ...
  toString: function toString(){
    ...
    var string = kernel32.GlobalLock(this._handle).readCString();
    kernel32.GlobalUnlock(this._handle);
    return string;
  },
...
var platform = module.exports = {
  ...
  write: function write(format, value){
    ...
    var buf = ffi.ref.alloc(ffi.ref.types.ulong);
